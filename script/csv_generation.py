...
fieldnames: list = ["ID", "titre", "date_imprimée",
                    "Date plus précise", "lieu", "nb pages", "Notice"]
...
def prepare_row(filepath: str, fieldnames=fieldnames):
    ...
    title: str = re.sub(pattern=r"\s+", repl=" ",
                        string=data_dict["entête"]["titre"])
    maz_id = (re.match(pattern=r"^.*(?=_)",
              string=(Path(filepath).parts)[-1])).group()  # type: ignore
    # pub place
    ...
    if isinstance(data_dict["entête"]["pubDate"], list):
        ...
        pub_date = unknown_pub_date if unknown_pub_date in pub_date.values(
        ) else data_dict["entête"]["pubDate"]["@when"]
    print(pub_date)
...
def make_csv_from_json_dir(dir_path: str, csv_file, mode: str = "w", fieldnames: list = fieldnames):
    ...
    for filepath in glob.glob(dir_path):
        make_csv_from_json(filepath=filepath,
                           fieldnames=fieldnames, csv_file=csv_file, mode="a")
    return


# make_csv_from_json(filepath=test_file, csv_file=csv_file)
make_csv_from_json_dir(dir_path=maz_json_dir,
                       csv_file=csv_file, mode="w", fieldnames=fieldnames)
